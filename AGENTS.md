> **This file is the canonical project instruction set for ALL coding agents**
> (Claude Code, Codex, and anything else that reads AGENTS.md). `CLAUDE.md`
> in this directory is a thin wrapper that imports this file and holds
> Claude-Code-only additions. Edit THIS file for project knowledge; never
> maintain parallel copies. Workspace-wide rules (safety hooks, values,
> standards) live in the workspace root — Claude Code loads them
> automatically; Codex is pointed there by `~/.codex/AGENTS.md`.
...
├── summaries.py                # Findings summary helpers
...
- Public release was a squashed initial commit on 2026-04-17; subsequent v2 analysis work has normal history. Force-push to main requires explicit user consent — don't just do it.
...
- Brian is not a coder by trade — end-of-turn summaries of what changed/what happened are wanted, not unwanted. Don't assume the diff speaks for itself.
